...
class GraphicsExtension extends ISLExtension {
    ...
    const context = this.#getRenderContext();
    {
      ...
      if (labels.includes("aligned")) {
        if (labels.includes("left")) {
          ...
          throw new ISLError(
            "'aligned' requires a direction: 'left' or 'right'",
            SyntaxError
          );
        }
      }
  ...
  #isl_text(labels, x, y, text, maxWidth = undefined) {
    ...
    const context = this.#getRenderContext();
    context.font =
      (this.#canvasSettings.textSize ?? 20) +
      "px " +
      (this.#canvasSettings.textFont ?? "Arial");
  ...
  #isl_bg(colour) {
    ...
    context.rect(
      0,
      0,
      this.#getCanvas().getAttribute("width"),
      this.#getCanvas().getAttribute("height")
    );
    context.fill();
  }
  #isl_draw() {
  ...
  #setupKeywords() {
    this.addKeyword(
      "canvas",
      function (interpreter, labels, width, height) {
    ...
    this.addKeyword(
      "rectangle",
      function (interpreter, labels, x, y, width, height) {
        this.#drawBuffer.push({
          type: this.#isl_rect,
          params: [labels].concat(
            [x, y, width, height].map((x) => (x ? x.value : undefined))
          ),
          options: structuredClone(this.#canvasSettings),
        });
      },
      [
        { type: "number", name: "x" },
    ...
    this.addKeyword(
      "circle",
      function (interpreter, labels, x, y, radius) {
        this.#drawBuffer.push({
          type: this.#isl_circle,
          params: [labels].concat(
            [x, y, radius].map((x) => (x ? x.value : undefined))
          ),
          options: structuredClone(this.#canvasSettings),
        });
      },
    ...
    this.addKeyword(
      "text",
      function (interpreter, labels, x, y, text, maxWidth) {
        this.#drawBuffer.push({
          type: this.#isl_text,
          params: [labels].concat(
            [x, y, text, maxWidth].map((x) => (x ? x.value : undefined))
          ),
          options: structuredClone(this.#canvasSettings),
        });
      },
    ...
    this.addKeyword(
      "background",
      function (interpreter, labels, colour) {
        this.#drawBuffer.push({
          type: this.#isl_bg,
          params: [colour.value],
          options: structuredClone(this.#canvasSettings),
    ...
    this.addKeyword(
      "textsize",
      function (interpreter, labels, size) {
    ...
    this.addKeyword(
      "outline",
      function (interpreter, labels, colour, width) {
        if (labels.includes("no")) {
          this.#canvasSettings.fillColour = "#00000000";
        } else {
  ...
  #checkCTX() {
    if (!this.#getRenderContext())
      throw new ISLError(
        "Cannot perform a graphics operation without a canvas!",
        SyntaxError
      );
  }
  #setupVariables() {
